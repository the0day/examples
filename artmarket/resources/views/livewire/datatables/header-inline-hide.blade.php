<div wire:click.prefetch="toggle('{{ $index }}')"
     class="@if($column['hidden']) relative table-cell h-10 w-3 bg-blue-100 hover:bg-blue-300 overflow-none align-top group @else hidden @endif"
     style="min-width:12px; max-width:12px"
>
    <button class="relative h-10 w-3 focus:outline-none">
        <span
...
<div
        class="@if($column['hidden']) hidden @else relative h-10 overflow-hidden align-top flex table-cell @endif"
    ...
    @if($column['unsortable'])
        <div
            ...
            <span
                    class="inline flex-grow @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @endif"">{{ str_replace('_', ' ', $column['label']) }}</span>
        </div>
    @else
        <button wire:click="sort('{{ $index }}')"
            ...
            <span class="inline text-xs text-blue-400">
                @if($sort === $index)
                    @if($direction)
                        <x-icons.chevron-up class="h-4 w-4 text-green-600 stroke-current"/>
                    @else
                        <x-icons.chevron-down class="h-4 w-4 text-green-600 stroke-current"/>
                    @endif
                @endif
            </span>
        </button>
