@unless($column['hidden'])
    <div class="relative table-cell h-10 overflow-hidden align-top"
        ...
        @else
            <button wire:click="sort('{{ $index }}')"
                ...
                @if($sort === $index)
                        @if($direction)
                            <x-icons.chevron-up wire:loading.remove class="h-4 w-4 text-green-600 stroke-current"/>
                        @else
                            <x-icons.chevron-down wire:loading.remove class="h-4 w-4 text-green-600 stroke-current"/>
                        @endif
                    @endif
            </span>
            </button>
